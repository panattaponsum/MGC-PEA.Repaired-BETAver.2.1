...
window.showActivityLogs = async function(direction = 'first') {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            html += `<tr class="hover:bg-slate-50 border-b border-slate-100 text-center"><td class="p-2 border text-sm whitespace-nowrap">${time}</td><td class="p-2 border text-sm break-words">${d.userEmail || 'System'}</td><td class="p-2 border text-sm break-words">${siteDisplay}</td><td class="p-2 border"><span class="px-2 py-0.5 rounded-full text-xs font-bold uppercase whitespace-nowrap ${badgeClass}">${d.action}</span></td><td class="p-2 border text-left text-sm text-slate-600 break-words">${d.details}</td></tr>`;
        });
